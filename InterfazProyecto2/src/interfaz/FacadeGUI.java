...
import Model.ControladorVentas;
...
public class FacadeGUI {
	...
	private ControladorVentas conV = new ControladorVentas();
	...
	private void windowError(String msg)
	{
		...
		ImageIcon iconoFrame = new ImageIcon("./IntefazProyecto2/data/warning.png");
	...
	public int initUserSesion(String user, String pass)
	{
	 ...
	 public void comprarJugador(String jug, EquipoFantasia eq)
	 {
		 eq.getPlantilla().put(jug, ControladorTemporada.JugadoresMap.get(jug));
		 eq.setPresupuestoDisponible(eq.getPresupuestoDisponible()-ControladorTemporada.JugadoresMap.get(jug).getValorCompra());
	 }
	 
	 public void venderJugador(String jug, EquipoFantasia eq)
	 {
		 eq.getPlantilla().remove(jug, ControladorTemporada.JugadoresMap.get(jug));
		 eq.setPresupuestoDisponible(eq.getPresupuestoDisponible()+ControladorTemporada.JugadoresMap.get(jug).getValorCompra());
	 }
	 ...
	 {
	conF.configurarAlineacion(jugadores, eq);
	 }
